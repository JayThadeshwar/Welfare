...
$sql = "SELECT * FROM `disaster_event` WHERE city = '".$ct."'";
...
if ($num > 0) {
    ...
    if($row["city"]==$ct){
    	...
    	$f=$row["flag"];
    	$f++;
    	$sql = "UPDATE `disaster_event` SET `flag`=".$f." WHERE `city`='".$ct."' AND `dis_name` = '".$dis."'";
    	//echo $sql;
    	if ($conn->query($sql) === TRUE) {
		    //echo "Record updated successfully";
		    if ($f>=3) {
		        $uni_rl="Location: http://localhost:8080/welfare/active_disaster.php?flag=".$f."&change=0&disaster=".$dis;	
		    }
		    else{
